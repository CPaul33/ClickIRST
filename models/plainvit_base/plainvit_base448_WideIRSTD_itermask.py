...
MODEL_NAME = 'WideIRSTD_plainvit_base448'
...
def train(model, cfg, model_cfg):
    ...
    trainset = WideIRSTD(
        ...
        samples_scores_path=cfg.WideIRSTD_PATH + '/train/WideIRSTD_train_weights.pkl',
        samples_scores_gamma=1.25  # Parameters for adjusting sample sampling bias
    )
    ...
    trainer = ISTrainer(model, cfg, model_cfg, loss_cfg,
                        ...
                        checkpoint_interval=[(0, 20), (50, 5)],
                        image_dump_interval=300,
                        metrics=[AdaptiveIoU()],
                        max_interactive_points=model_cfg.num_max_points,
                        max_num_next_clicks=3)
    # Two-stage training: first focus on easy/medium samples, then switch to hard samples.
    if hasattr(trainset, 'set_sampling_phase'):
        trainset.set_sampling_phase(1)
    trainer.run(num_epochs=55, validation=False)

    if hasattr(trainset, 'set_sampling_phase'):
        trainset.set_sampling_phase(2)
    trainer.run(num_epochs=70, start_epoch=55, validation=False)
