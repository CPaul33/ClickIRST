...
def save_checkpoint(net, checkpoints_path, epoch=None, prefix='', verbose=True, multi_gpu=False, train_loss=None):
    if epoch is None:
...
def get_labels_with_sizes(x):
    ...
    # print("obj_sizes:", obj_sizes)
    labels = np.nonzero(obj_sizes)[0].tolist()
    labels = [x for x in labels if x != 0]
    # print("labels:", labels)
    # print("obj_sizes[labels].tolist:", obj_sizes[labels].tolist())
