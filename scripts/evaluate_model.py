...
def parse_args():
    ...
    group_iou_thresh.add_argument('--target-iou', type=float, default=0.90,
                                  help='Target IoU threshold for the NoC metric. (min possible value = 0.7)')
    ...
    parser.add_argument('--postproc-edge-thresh', type=float, default=0.9,
                        help='Edge threshold ratio used in post-processing (default: 0.9).')
    parser.add_argument('--postproc-compare', action='store_true', default=False,
                        help='Evaluate both with and without post-processing and log results for comparison.')
    ...
    if (args.iou_analysis or args.print_ious) and args.min_n_clicks <= 1:
        ...
        args.target_iou = max(0.7, args.target_iou)
        ...
        for checkpoint_path in checkpoints_list:
            ...
            if args.postproc_compare:
                dataset_results_off = evaluate_dataset(dataset, predictor, pred_thr=args.thresh,
                                                       ...
                                                       edge_threshold=args.postproc_edge_thresh)
                dataset_results_on = evaluate_dataset(dataset, predictor, pred_thr=args.thresh,
                                                      max_iou_thr=args.target_iou,
                                                      min_clicks=args.min_n_clicks,
                                                      max_clicks=args.n_clicks,
                                                      callback=vis_callback,
                ...
                dataset_results = evaluate_dataset(dataset, predictor, pred_thr=args.thresh,
                                                   ...
                                                   max_clicks=args.n_clicks,
                                                   callback=vis_callback,
                                                   postproc_enabled=args.postproc_enable,
                                                   edge_threshold=args.postproc_edge_thresh)
            ...
            if args.postproc_compare:
                ...
                save_results(args, row_name_base + '_PP=off', dataset_name, logs_path, logs_prefix, dataset_results_off,
                ...
                if args.iou_analysis:
                    save_iou_analysis_data(args, dataset_name, logs_path,
                                           ...
                                           model_name=args.model_name)
                save_results(args, row_name_base + '_PP=on', dataset_name, logs_path, logs_prefix, dataset_results_on,
                             save_ious=single_model_eval and args.save_ious,
                             single_model_eval=single_model_eval,
                             print_header=print_header)
                print_header = False
            else:
                row_name = row_name_base + ('_PP=on' if args.postproc_enable else '_PP=off')
                if args.iou_analysis:
                    save_iou_analysis_data(args, dataset_name, logs_path,
...
def save_results(args, row_name, dataset_name, logs_path, logs_prefix, dataset_results,
    ...
    header, table_row = utils.get_results_table(noc_list, over_max_list, row_name, dataset_name,
                                                ...
                                                first_click_miou=first_click_miou)
    ...
    if args.print_ious:
        ...
        if target_iou_int not in [70, 80, 90]:
